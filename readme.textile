...
to a function, JSON.encoding all of a function's return values,
etc. They are about as a powerful a construct as you can have without
...
# A function arguments will be checked to see if any unrealized Promises
...
function that has also been decorated by promise- fnB for example.

<pre>
fnB(get('a'), 'b')
</pre>

This sends the resultin Promise instance of <code>get('a')</code> to
fnB which will force it to realized. However since fnB is blocking
*it returns its own Promise for its final return value*. Note that the
above logic means that a lot of Promise instances are generated. For
completeness fnB itself returns a Promise when it runs!  The *promise*
decorator is smart enough to see that the function finaly returned a
value but that value is itself an unrealized promise, it will continue
to block until it can get a real value.
