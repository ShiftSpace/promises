...
Compare this to the example above where the JavaScript definition of
...
the request. So get can only return the Request object not send
it. You now have send the request yourself after you've set up your
...
requirements become more complex your code will become harder and harder
to maintain.
...
code. Before we consider the solution take note of another downside of
callback oriented AJAX requests. 
...
user's of this library extension. There's no way to solve this,
callback's on function interfaces are viral!
...
checking arguments to a function, JSON.encoding all of a function's
return values, etc. They about a powerful a construct you can have
without full blown Lisp macros.

In addition to the Promises class the Promises library provides a
...
# A function arguments will checked to see if any unrealized Promises
...
# This means all arguments inside of promise decorated function will
...
Notice we have no callbacks. Yes this code makese 6 simultaneous
Requests. We don't have to queue them.  When we run this code will
...
function that has also be decorated by promise- fnB for example.

<pre>
fnB(get('a'), 'b')
</pre>

Send the result of <code>get('a')</code> to fnB will force the Promise
instance to be realized. However since it fnB is blocking it returns
a Promise for it's return value. Note that the above logic means that
a lot of Promise instances are generated. For completeness fnB itself
returns a Promise when it runs! The Promise decorator is smart enough
to see that the function returned a value that is itself a promise, it
will continue to block until it can get a real value.

Also note the use of Promise.op.

# Promise.op allows you to modify the value of a Promise.
# If a Promise is unrealized Proimse.op returns the Promise.
# If realized Promise.op returns the value after the op is applied.
# Before the Promise fires it's realized event all ops that were
...
Hopefully yout beginning to see that regular requests cannot be composed
and Promises can.
...
# At the creation site of a Promise instance you can modify the
  instance at will.
# As soon as Promise is sent to another function, you call op at your
  own peril.
...
that return promises. Consider the following flawed example:
